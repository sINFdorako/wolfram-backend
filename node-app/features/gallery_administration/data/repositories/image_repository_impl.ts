...
export class ImageRepositoryImpl implements ImageRepository {
  ...
  constructor() {
    this.dataSource = new ImageDataSource();
  }

  async getImagesByLandingPageAndType(
    userId: number,
    landingpageId: number,
    imageType: ImageType
  ): Promise<Image[]> {
    const images = await this.dataSource.findByLandingpageIdAndImageType(
      userId,
      landingpageId,
      imageType
    );
  ...
  async getImagesByUserAndCategory(
    userId: number,
    categoryId: number
  ): Promise<Image[]> {
    const images = await this.dataSource.findImagesByUserIdAndCategory(
      userId,
      categoryId
    );
  ...
  async createImage(image: Image): Promise<Image> {
    const newImage = await this.dataSource.createImage(
      imageEntityToModel(image)
    );
    return newImage;
  }

  async updateImage(image: Image, imageId: number, userId: number): Promise<Image> {
    await this.dataSource.updateImage(imageEntityToModel(image), imageId, userId);
